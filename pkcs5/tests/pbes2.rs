...
/// [bee2], the reference implementation. The container's password is `zed`.
///
...
/// Decoding test for PBES2 + PBKDF2-HMAC-HBELT + belt-kwp `AlgorithmIdentifier`
#[test]
#[cfg(feature = "belt")]
fn decode_pbes2_pbkdf2_hmac_hbelt_belt_kwp() {
    let scheme = pkcs5::EncryptionScheme::try_from(PBES2_PBKDF2_HBELT_BELT_KWP_ALG_ID).unwrap();
    let params = scheme.pbes2().unwrap();

    let pbkdf2_params = params.kdf.pbkdf2().unwrap();
    assert_eq!(pbkdf2_params.salt.as_bytes(), &hex!("0c7f910ebb684785"));
    assert_eq!(pbkdf2_params.iteration_count, 10000);
    assert_eq!(pbkdf2_params.key_length, None);
    assert_eq!(pbkdf2_params.prf, pbes2::Pbkdf2Prf::HmacHbelt);

    // `belt-kwp` carries no IV: it is decoded from NULL parameters.
    assert_eq!(params.encryption, pbes2::EncryptionScheme::BeltKwp);
}

/// Encoding test for PBES2 + PBKDF2-HMAC-HBELT + belt-kwp `AlgorithmIdentifier`
#[test]
#[cfg(feature = "belt")]
fn encode_pbes2_pbkdf2_hmac_hbelt_belt_kwp() {
    let mut buffer = [0u8; 1024];
    ...
    let encoded_der = encoder.finish().unwrap();
    assert_eq!(encoded_der, PBES2_PBKDF2_HBELT_BELT_KWP_ALG_ID);
}
