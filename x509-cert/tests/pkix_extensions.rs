    ...
    for ext in exts {
        match ext.extn_id {
            SubjectDirectoryAttributes::OID => {
                ...
                assert_eq!(ext.extn_value.as_bytes(), decoded.to_vec().unwrap());
            }
...
fn decode_cert() {
    ...
    for ext in i {
        if 0 == counter {
            ...
            assert_eq!(ext.extn_value.as_bytes(), reencoded);
        } else if 1 == counter {
                ...
                if 18 == cp_counter {
                    ...
                    for pqi in pq.iter() {
                        if 0 == counter_pq {
                            ...
                            let cpsval = pqi.qualifier.as_ref().unwrap().ia5_string().unwrap();
                            assert_eq!(
