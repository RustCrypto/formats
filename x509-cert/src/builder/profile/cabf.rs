...
pub fn check_names_encoding(name: &Name, multiple_allowed: bool) -> Result<()> {
    ...
    for rdn in name.0.iter() {
        if rdn.0.len() != 1 {
            todo!("Each RelativeDistinguishedName MUST contain exactly one AttributeTypeAndValue.");
        }

        for atv in rdn.0.iter() {
            if !multiple_allowed {
                if !seen.insert(atv.oid.clone()) {
                    todo!(
...
allowed in these Requirements"
            ...
            if ordering.find(|attr| (*attr) == &atv.oid).is_none() {
                todo!("non-ordered attribute, or disallowed attribute");
            }
        }
    }
...
pub fn ca_certificate_naming(subject: &Name) -> Result<()> {
    ...
    for rdn in subject.0.iter() {
        for atv in rdn.0.iter() {
            if !allowed.remove(&atv.oid) {
                todo!("Element not allowed or duplicated.");
            }
    ...
    if !required.is_empty() {
        todo!("Not all required elements specified.");
    }
