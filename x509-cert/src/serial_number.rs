...
mod tests {
    ...
    fn serial_number_generate() {
        let sn = SerialNumber::<Rfc5280>::generate(&mut rand::rng()).unwrap();
        ...
        let sn =
            SerialNumber::<Rfc5280>::generate_with_prefix(&[], 8, &mut rand::rng()).unwrap();
        assert!(matches!(sn.as_bytes().len(), 8..=9));

        let sn =
            SerialNumber::<Rfc5280>::generate_with_prefix(&[1, 2, 3], 8, &mut rand::rng())
                .unwrap();
