...
#[derive(Clone, Debug, Eq, PartialEq, Defer)]
pub struct SigDeferCertificate<'a> {
    /// tbsCertificate       TBSCertificate,
    pub tbs_certificate: TBSCertificate<'a>,
    /// signatureAlgorithm   AlgorithmIdentifier,
    pub signature_algorithm: AlgorithmIdentifier<'a>,
...
///Structure supporting deferred decoding of serial_number field in the TBSCertificate SEQUENCE
pub struct XCoverage1TBSCertificate<'a> {
    /// Decoded field
...
impl<'a> Decodable<'a> for XCoverage1TBSCertificate<'a> {
    fn decode(decoder: &mut Decoder<'a>) -> der::Result<XCoverage1TBSCertificate<'a>> {
...
pub fn xcoverage1_tbscertificate(der_encoded: &[u8]) {
    let defer_result = XCoverage1TBSCertificate::from_der(der_encoded);
    let defer = defer_result.unwrap();
    let full_result = TBSCertificate::from_der(der_encoded);
    let full = full_result.unwrap();
    let reencoded = full.serial_number.to_vec().unwrap();
    assert_eq!(defer.serial_number, reencoded);
}
...
fn reencode_cert() {
    ...
    // these three functions do deferred decode/reencode tests for fields indicated by index number in name
    coverage0_certificate(der_encoded_cert);
    coverage1_certificate(der_encoded_cert);
    coverage2_certificate(der_encoded_cert);

    // this function calls the above three (or n for structures with more than three)
    coverage_certificate(der_encoded_cert);

    let result3 = DeferSigAlgDeferCertificate::from_der(der_encoded_cert);
    let cert3 = result3.unwrap();

    let result4 = DeferSigDeferCertificate::from_der(der_encoded_cert);
    let cert4 = result4.unwrap();
    ...
    let reencoded_sig = cert.signature.to_vec().unwrap();
    assert_eq!(cert4.signature, reencoded_sig);
